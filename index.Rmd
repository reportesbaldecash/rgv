...
datos_flujo <- cargar_datos_flujo()

solicitudes_backup <- solicitudes

solicitudes <- solicitudes %>% 
    mutate(
        Categoria.RGV = ifelse(Source %not in% c('solicitud', 'rrss'), 
                               'Convenios',
                               ifelse(Redes == 'SÍ',
                                      'Redes',
                                      'Organico'))
...
solicitudes_analisis <- solicitudes %>% 
    filter(
        Fecha >= '2022-01-01',
        (!str_detect(Motivos_prerechazo, "Duplicado") | is.na(Motivos_prerechazo))
    ) %>%
...
leads <- read.csv('Landing Page-General.csv') #my wd
...
solicitudes_resumen <- solicitudes_resumen %>% 
  slice(1:nrow(.)) # eliminar la última fila. Es necesario?
...
solicitudes_analisis %>% 
    filter(Fecha == as_date("2024-05-16")) %>% 
    View()
```
