...
```{r}
# contar la cantida de días de L a V de todo el mes que no sean feriados
...
```{r, include=FALSE}
proyeccion_lineal_cifra <- max(firmas_resumen2$acumulado_con_proyeccion, na.rm = TRUE)
...
theme_balde <- function() {
  ...
  theme_bw(base_size = 15) +
    ...
    theme(plot.subtitle = element_markdown(color = color_text, size = 11, hjust = 0.5, face = "bold", margin=margin(0,0,22,0))) +
...
```{r}
proyeccion_firmas_plot <- firmas_resumen2 %>%
  filter(Fecha.de.Firma >= inicio_mes_hoy) %>%
  ggplot(aes(x = Fecha.de.Firma, y = acumulado), 
         color = color_indigo) +
  geom_line(color = color_turquesa) +
  geom_line(data = firmas_resumen2 %>% filter(Fecha.de.Firma >= fecha_hoy),
...
trafico_solicitudes_plot <- interesados_resumen %>%
  filter(Fecha >= inicio_mes_hoy & Fecha < fecha_hoy + 1 ) %>% 
...
firmas_totales_plot <- firmas_resumen2 %>%
  filter(Fecha.de.Firma >= inicio_mes_hoy & Fecha.de.Firma <= fecha_hoy) %>% 
